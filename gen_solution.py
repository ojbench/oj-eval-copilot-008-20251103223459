...
def generate_solution(problem_id, mv_file):
    with open(mv_file, 'r') as f:
        mv_code = f.read()
    
    # Escape the .mv code for C string
    escaped = mv_code.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n"\n        "')
    
    c_code = f'''// Solution for problem {problem_id}
// Auto-generated from {mv_file}
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: gen_solution.py <problem_id> <mv_file>")
        sys.exit(1)
    
    problem_id = sys.argv[1]
    mv_file = sys.argv[2]
    
    print(generate_solution(problem_id, mv_file))
