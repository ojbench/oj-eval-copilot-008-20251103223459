// Combined solution for problem 2076
// Includes interpreter + hardcoded .mv program
...
int main(int argc, char ** argv) {
    // Create temp file with .mv program
    char template[] = "/tmp/mv_XXXXXX";
    ...
    write(fd, mv_program, strlen(mv_program));
    close(fd);
    
    // Open program file
    if (!(program = fopen(template, "r"))) {
